<template>
	<view class="main">
		...
		<!-- 用户信息 -->
		<u-popup v-model="userCardShow" mode="bottom" border-radius="34" length="50%" :closeable="true">
				<UserCard :id="fid" :name="fname" :portrait="fportrait"></UserCard>
		</u-popup>
		<u-navbar class="nvabar" :is-fixed="true">
			<view class="nvabar-main">
				...
				<view class="nvabar-main-icon">
					<u-icon class="icon-camera" name="phone-fill" size="36"></u-icon>
		...
		<view class="box-1" :class="{'listBoxPadding':listBoxPadding}" id="list-box" @click="inputFocus">
			<view class="talk-list">
				<view v-for="(item,index) in talkList" :key="index" :id="`msg-${item.id}`">
					<view class="item flex_col" :class=" item.uid == fid ? 'pull':'push' "
		...
		methods: {
			...
			link() {
				this.chatSocket = uni.connectSocket({
					url: this.wsUrl + '?type=chat_uid_' + this.fid + '&token=' + uni.getStorageSync('token'),
					complete: () => {}
				});
			...
			setCollect() {
				...
				this.$u.post('Collect/setCollect', {
