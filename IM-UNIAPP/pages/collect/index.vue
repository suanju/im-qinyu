<template>
	<view class="main">
		...
		<!-- 删除收藏提示 -->
		<u-action-sheet :list="[{text: '删除',color: '#FF3300',fontSize: 24}]" @click="setCollect()"
		...
		<view>
			<u-navbar class="nvabar" :is-back="true" :is-fixed="true">
				...
				<u-icon  class="nvabar-icon" @click="redact" name="list" size="36"></u-icon>
			</u-navbar>
		</view>
		...
		<view class="collect">
			<view class="collect-item" v-for="(item,idnex) in collectList" :key="item.id" @touchstart="getTouchStartItem(item.id)" @touchend="getTouchendItem()">
				<view class="collect-item-head">
					...
					<view class="collect-item-text">
						...
						<view class="collect-item-text-collect" v-if="item.collect_type == 'text'">{{item.collect}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//调用删除接口
			setCollect(){
				this.$u.post('collect/delCollect', {
					id : this.collectID
				}).then(res => {
					console.log(res)
					if (res.status == 200) {
						this.collect = this.collect.filter((e)=>{
							if(e.id != this.collectID){
								return e
							}
						})
						this.$refs.uToast.show({
